...
#根据不同的imgembedding 进行了修改，要对以下代码进行修改
def load_from(sam, state_dict, image_size, vit_patch_size):
    sam_dict = sam.state_dict()
    # new_state_dict=sam_dict
    except_keys = ['mask_tokens', 'output_hypernetworks_mlps', 'iou_prediction_head']
    new_state_dict = {k: v for k, v in state_dict.items() if
                      k in sam_dict.keys() and except_keys[0] not in k and except_keys[1] not in k and except_keys[2] not in k}#不包括以上三种，创建一个新的字典
    pos_embed = new_state_dict['image_encoder.pos_embed']
    token_size = int(image_size // vit_patch_size)
    if pos_embed.shape[1] != token_size:#修改image_encoder的pos_embed
